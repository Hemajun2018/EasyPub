import { ImagePlan, StyleType } from './types';

const API_ROUTE = '/api/formatter/generate';
...
async function callEvolink(body: any) {
...
async function generateViaEvolink(
  ...
  const data = await callEvolink({ ...common, traceId, turn: 1 });
  const parts = data?.candidates?.[0]?.content?.parts || [];
  const text = parts.map((p: any) => p?.text).filter(Boolean).join('');
  console.log(
    `[TRACE] turn=1 trace=${traceId} dur=${Date.now() - started}ms outChars=${text.length}`
  );
...
async function generateTextViaEvolink(userText: string, opts?: GenOptions) {
  ...
  const data = await callEvolink(body);
...
/**
 * Analyze a sample article's HTML (inner of #js_content) and return a style prompt and image wrapper hints.
 * The result is used as a custom instruction template to reproduce the style.
 */
export const analyzeHtmlToTemplate = async (html: string): Promise<{ prompt: string; palette?: Record<string, string>; imageBlock?: { wrapperStyle: string; imgStyle: string }; notes?: string; }> => {
  const systemText = `You are a senior WeChat typesetting analyst. Output STRICT JSON only.`;
  const schema = {
    type: 'object',
    properties: {
      prompt: { type: 'string' },
      ...
      imageBlock: {
        ...
        required: ['wrapperStyle', 'imgStyle']
      },
      notes: { type: 'string' },
    },
    required: ['prompt']
  ...
  const instruction = `
Analyze the following HTML fragment from a WeChat Official Account article. It may be the inner of #js_content (main body) or #page-content (the whole content region). Identify the PRIMARY ARTICLE SKELETON and produce a CUSTOM STYLE PROMPT (a single instruction string) that can format NEW articles to FAITHFULLY match this style.

What the returned prompt MUST include (combine precision + flexibility):
1) HARD RULES (tell the formatter explicitly):
   - Output CONTENT STREAM ONLY (no <html>, <head>, <body>); NO <style> tags; NO class attributes.
   - INLINE CSS ONLY; use rgb() colors.
   - IMAGE TOKENS MUST BE KEPT: [[IMAGE:img-...]], {{IMG:img-...}}, {{IMGURL:https://...}}, [[URL:n]]. Never convert tokens to <img>; you may WRAP the token with a simple <section> for layout.
   - Single root wrapper is recommended; ensure all tags are closed; end with <!-- END_OF_ARTICLE -->.

2) GLOBAL CONTAINER:
   - Provide one outer <section style="..."> specifying font family, base font-size, line-height, letter-spacing, color, text-align, and (if present in the sample) background/rounded corners/shadows/linear-gradients.

3) PALETTE (explain inline in the prompt):
   - Identify main accent color(s), text color, muted text, and card background (e.g., ACCENT, TEXT, MUTED, BG_CARD) as rgb().
   - In the prompt, refer to these colors explicitly with their rgb() values (no variables, just concrete styles).

4) COMPONENT LIBRARY WITH TRIGGERS (each component includes PURPOSE, WHEN-TO-USE, and an EXACT HTML SNIPPET with inline CSS — NO classes). Cover at least:
   - Top Intro Bar (optional): for "关注/星标/引导" lines near the top.
   - L-Shaped Corner + Grey Card (optional): a 2.25em corner (border-top/left) with negative margin overlay and a BG_CARD panel for highlights.
   - Section Heading (optional): either centered title with a bottom border OR numbered headings like 01/02..., specify alignment, font sizes, and separators implemented using CSS (linear-gradient/borders), NOT <svg>.
   - Standard Paragraphs (required): consistent margins and text styles (<section> + inner <p> with margin:0; padding:0;).
   - Quote/Note Box (optional): left dotted/solid border using ACCENT, padding, and font sizing.
   - List Box (optional): outer solid border with ACCENT and per-item symbol (e.g., ❖) layout.
   - Image Containers (token wrappers): provide two variants — 80% width with shadow; 100% width (full-bleed) without shadow — but REMIND that the inner content remains the token.
   - Footer Author/Editor Info (optional): muted small text lines.

5) ACTIVATION PRINCIPLES (write them clearly in the prompt):
   - Use a component ONLY IF the content triggers it (e.g., presence of certain phrases or structure). Do not fabricate.
   - Keep same type elements visually consistent across the article.
   - Use <p><br/></p> to add subtle spacing when needed; avoid excessive blank lines.

Additionally return:
- imageBlock (JSON): recommended { wrapperStyle, imgStyle } describing how <img> should look when CLIENT replaces tokens (e.g., centered with 80% width and shadow).
- palette (JSON): { ACCENT?, TEXT?, MUTED?, BG_CARD?, ... } with rgb() values.
- notes (JSON): any special observations about the style.

IMPORTANT: Return STRICT JSON only with keys { prompt, imageBlock, palette, notes }.
HTML SAMPLE START\n${html.substring(0, 60000)}\nHTML SAMPLE END
`;

  const text = await generateTextViaEvolink(instruction, {
    systemText,
    temperature: 0.2,
    maxOutputTokens: 8192,
  ...
  try {
    ...
    const palette = parsed?.palette && typeof parsed.palette === 'object' ? parsed.palette : undefined;
    const imageBlock = parsed?.imageBlock && typeof parsed.imageBlock === 'object'
      ? { wrapperStyle: String(parsed.imageBlock.wrapperStyle || '').trim(), imgStyle: String(parsed.imageBlock.imgStyle || '').trim() }
      : undefined;
    const notes = parsed?.notes ? String(parsed.notes) : undefined;
    return { prompt, palette, imageBlock, notes };
  } catch (e) {
    // Fallback: wrap as plain stylePrompt
    return { prompt: text?.trim() || '' };
  }
};

/**
 * Format text using a custom stylePrompt (instruction template) instead of built-in StyleType.
 */
export const formatTextWithStylePrompt = async (text: string, stylePrompt: string): Promise<string> => {
  const IMAGE_TOKEN_RULES = `
    IMAGE TOKEN RULES (STRICT):
    - The input may contain tokens: [[IMAGE:img-<id>]] (preferred), {{IMG:img-<id>}}, {{IMGURL:https://...}}, and short URL tokens [[URL:1]], [[URL:2]], etc.
    - Do NOT remove, reorder, rename, or deduplicate tokens.
  ...
  const prompt = `
    ...
    ENFORCERS:
    - Use the component library ONLY WHEN TRIGGERED by the input content; do not output modules that are not applicable.
    - Do not invent new content or metadata; preserve the user's narrative order.
    - Keep consistent styles for the same component type across the article.
    - Output ONLY the content stream (no <html>/<head>/<body>), and ensure a single coherent wrapper.
...
`;
  ...
  while (safetyBreak < MAX_TURNS) {
    ...
    if (combined.includes('<!-- END_OF_ARTICLE -->')) break;
    messages.push({ role: 'model', parts: [{ text: chunkSafe }] });
    messages.push({ role: 'user', parts: [{ text: 'Continue ONLY the remaining HTML from where you left off. Do not repeat any previous content. Ensure all tags are closed. End with <!-- END_OF_ARTICLE -->.' }] });
    safetyBreak += 1;
  }
