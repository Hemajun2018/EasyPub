...
export function Header({ header }: { header: HeaderType }) {
  ...
  const NavMenu = () => {
    return (
      ...
      >
        <NavigationMenuList className="gap-2 text-sm font-medium text-foreground/70">
              ...
              return (
                <NavigationMenuLink key={idx} asChild>
                  <Link
                    ...
                    className={cn(
                      'flex items-center gap-2 rounded-full px-4 py-2 transition hover:bg-white/70 hover:text-foreground',
  ...
  return (
    <>
      <header
        ...
        className="fixed inset-x-0 top-0 z-50 has-data-[state=open]:h-screen has-data-[state=open]:backdrop-blur"
      >
        <div
          className={cn(
